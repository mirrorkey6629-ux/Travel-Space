...
const hotelPayload = (city: City) => ({ hotel: city.hotel, hotelUrl: city.hotelUrl, hotelCheckInTime: city.hotelCheckInTime ?? '', hotelCheckOutTime: city.hotelCheckOutTime ?? '', hotelNotes: city.hotelNotes ?? '' })
const emptyCity = (): City => ({ id: uid(), name: '', arrival: '', departure: '', arrivalPeriod: 'morning', departurePeriod: 'evening', hotel: '', hotelUrl: '', hotelCheckInTime: '', hotelCheckOutTime: '', hotelNotes: '', trainIn: '', trainOut: '', transportIn: emptyTransport(), transportOut: emptyTransport(), places: {}, tasks: [], files: [] })
  ...
  return (
    <aside className="sidebar">
      <section className="glass sidebar-card trip-summary">
        ...
        <div className="city-list sidebar-city-list">{trip.cities.map((city, index) => {
          const previousCity = trip.cities[index - 1]
          const incomingCity = previousCity && (previousCity.transportOut?.type || previousCity.trainOut) ? previousCity : city
          const incomingDirection = incomingCity === city ? 'in' : 'out'
          const incomingDetails = incomingDirection === 'in' ? city.transportIn : previousCity?.transportOut
          const incomingTicket = incomingDirection === 'in' ? city.trainIn : previousCity?.trainOut ?? ''
          const incomingLabel = `Билеты в ${city.name}`
          return (
            <div className={`sidebar-city-group${city.id === selectedCityId ? ' selected' : ''}`} key={city.id}>
              <CityRow city={`${city.id === selectedCityId ? '📌 ' : ''}${city.name}`} dates={formatShortRange(city.arrival, city.departure)} duration={formatDays(cityDays(city))} image={city.imageUrl || cityPlaceholder} imageAlt="Изображение города" selected={city.id === selectedCityId} aria-current={city.id === selectedCityId ? 'true' : undefined} onClick={() => onCity(city)} />
              <div className="sidebar-city-documents">
                <DocumentStatus checked={isTransportComplete(incomingDetails, incomingTicket)} onClick={() => onTransport(incomingCity, incomingDirection)}>{withTransportEmoji(incomingLabel, incomingDetails?.type)}</DocumentStatus>
                <DocumentStatus checked={isHotelComplete(city)} onClick={() => onHotel(city)}>Отель</DocumentStatus>
                {index === trip.cities.length - 1 && <DocumentStatus checked={isTransportComplete(city.transportOut, city.trainOut)} onClick={() => onTransport(city, 'out')}>{withTransportEmoji('Отъезд', city.transportOut?.type)}</DocumentStatus>}
              </div>
            </div>
          )
        })}</div>
    ...
    <div className="overlay" role="presentation">
      <form className="glass modal editor transport-dialog" role="dialog" aria-modal="true" aria-labelledby="transport-title" onSubmit={(event) => { event.preventDefault(); if (draft.type) onSave(draft) }}>
        ...
        <div className="transport-ticket-list">
          ...
          {ticketCount < 2 && <InfoRow className="transport-ticket-row transport-ticket-row-empty" image={`${import.meta.env.BASE_URL}assets/ticket-placeholder.png`} imageAlt="Билет" title="Прикрепить билет" subtitle="Лучше в PDF формате" onClick={onTicket} actions={[{ icon: <Icon name="add-plus" />, label: 'Прикрепить билет', onClick: onTicket }]} />}
        </div>
  ...
  return (
    <div className="overlay" role="presentation">
      <form className="glass modal editor transport-dialog hotel-dialog" role="dialog" aria-modal="true" aria-labelledby="hotel-title" onSubmit={(event) => { event.preventDefault(); onSave({ name: draft.name.trim(), url: draft.url.trim(), checkInTime: draft.checkInTime, checkOutTime: draft.checkOutTime, notes: draft.notes.trim() }) }}>
        ...
        <div className={booking ? 'document-card-group' : ''}><InfoRow className={booking ? 'transport-ticket-row' : 'transport-ticket-row transport-ticket-row-empty'} image={`${import.meta.env.BASE_URL}assets/hotel-placeholder.png`} imageAlt="Отель" title={booking ? 'Бронь отеля' : 'Прикрепить бронь'} subtitle={booking ? booking.name : 'Лучше в PDF формате'} onClick={booking ? onOpenBooking : onBooking} actions={booking ? [{ icon: <Icon name="download" />, label: 'Скачать бронь', onClick: onDownloadBooking }, { icon: <Icon name="delete-forever" />, label: 'Удалить бронь', onClick: onDeleteBooking }] : [{ icon: <Icon name="add-plus" />, label: 'Прикрепить бронь', onClick: onBooking }]} />{booking && documentMetadata(booking) && <p className="document-card-metadata">{documentMetadata(booking)}</p>}</div>
...
function CityPanel({ city, previousCity, nextCity, initialPanel, onChange, onAddPlace, onUpdatePlace, onTrainChange, onHotelChange, onOpenDocument, onDownloadDocument, onDeleteDocument, onClose }: { city: City; previousCity?: City; nextCity?: City; initialPanel?: 'hotel' | 'in' | 'out' | null; onChange: (city: City) => void; onAddPlace: (city: City, date: string, place: Place) => void; onUpdatePlace: (city: City, date: string, place: Place) => void; onTrainChange: (city: City, direction: 'in' | 'out', file: TravelFile, source: File) => Promise<TravelFile | undefined>; onHotelChange: (city: City, file: TravelFile, source: File) => Promise<TravelFile | undefined>; onOpenDocument: (file: TravelFile) => void; onDownloadDocument: (file: TravelFile) => void; onDeleteDocument: (file: TravelFile) => Promise<void>; onClose: () => void }) {
  ...
  const saveHotel = (hotel: HotelDetails) => {
    let next = { ...draft, hotel: hotel.name, hotelUrl: hotel.url, hotelCheckInTime: hotel.checkInTime, hotelCheckOutTime: hotel.checkOutTime, hotelNotes: hotel.notes }
    if (hotel.name.trim()) {
      const arrivalPlaces = next.places[next.arrival] ?? []
      const previousUrl = draft.hotelUrl.trim()
      const previousName = draft.hotel.trim()
      const existing = arrivalPlaces.find((item) =>
        (previousUrl && item.url.trim() === previousUrl)
        || (hotel.url.trim() && item.url.trim() === hotel.url.trim())
        || (previousName && item.name.trim() === previousName),
      )
      if (existing) {
        const place = { ...existing, name: hotel.name.trim(), url: hotel.url.trim(), ...(coordinatesFromGoogleMapsUrl(hotel.url) ?? {}) }
        next = { ...next, places: { ...next.places, [next.arrival]: arrivalPlaces.map((item) => item.id === place.id ? place : item) } }
        onUpdatePlace(next, next.arrival, place)
      } else {
        const place = { id: uid(), name: hotel.name.trim(), url: hotel.url.trim(), ...(coordinatesFromGoogleMapsUrl(hotel.url) ?? {}) }
        next = { ...next, places: { ...next.places, [next.arrival]: [...arrivalPlaces, place] } }
        onAddPlace(next, next.arrival, place)
      }
    }
    ...
    <>
      <section className="glass city-page-card setup-transition">
        ...
        <div className="info-strip">
          <InfoRow title="Твой отель" subtitle={[draft.hotelCheckInTime, draft.hotel].filter(Boolean).join(' · ') || 'Где будем жить'} onClick={() => setHotelOpen(true)} actions={hotelDocument ? [{ icon: <Icon name="docs" />, label: 'Открыть бронь отеля', onClick: () => onOpenDocument(hotelDocument) }] : []} />
...
function Dashboard({ trip, user, tripCount, onChange, onEdit, onTrips, onProfile, onInvite, onCityChange, onAddPlace, onUpdatePlace, onTrainUpload, onHotelUpload, onDocumentDelete }: { trip: Trip; user: CurrentUser | null; tripCount: number; onChange: (trip: Trip) => void; onEdit: () => void; onTrips: () => void; onProfile: () => void; onInvite: () => void; onCityChange: (city: City) => void; onAddPlace: (city: City, date: string, place: Place) => void; onUpdatePlace: (city: City, date: string, place: Place) => void; onTrainUpload: (city: City, direction: 'in' | 'out', file: File) => Promise<TravelFile | undefined>; onHotelUpload: (city: City, file: File) => Promise<TravelFile | undefined>; onDocumentDelete: (file: TravelFile) => Promise<void> }) {
  ...
  return (
    <main className="screen trip-background dashboard" style={tripBackgroundStyle(trip)}>
      <TripSidebar trip={trip} user={user} tripCount={tripCount} selectedCityId={selectedCityId} onCity={(city) => { setSelectedCityId(city.id); setSelectedPanel(null) }} onHotel={(city) => { setSelectedCityId(city.id); setSelectedPanel('hotel') }} onTransport={(city, direction) => { setSelectedCityId(city.id); setSelectedPanel(direction) }} onEdit={onEdit} onInvite={onInvite} onTrips={onTrips} onProfile={onProfile} />
...
export default function App() {
  ...
  const updateCity = async (city: City) => {
    if (!trip?.id) return
    const routePayload = trip.role === 'owner' ? { name: city.name, arrivalDate: city.arrival, departureDate: city.departure, arrivalPeriod: city.arrivalPeriod, departurePeriod: city.departurePeriod } : {}
    try { await api.updateCity(trip.id, city.id, { ...routePayload, ...hotelPayload(city), ...transportPayload(city) }); setError('') }
