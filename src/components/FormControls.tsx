...
export function Input({ icon, label, theme = 'default', showIcon = true, showLabel = true, controlClassName = '', fieldClassName = '', ...props }: InputProps) {
  const control = (
    <span className={`form-control form-control-${theme}${showIcon && icon ? ' form-control-with-icon' : ''}${controlClassName ? ` ${controlClassName}` : ''}`}>
